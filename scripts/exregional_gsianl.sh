...
cp $nmmb_nems_bias/${RUN}.t${CYCrun}z.satbias.${tmmark_prev} ./satbias_in
err1=$?
if [ $err1 -ne 0 ] ; then
  cp $GESROOT_HOLD/satbias_in ./satbias_in
fi
cp $nmmb_nems_bias/${RUN}.t${CYCrun}z.satbias_pc.${tmmark_prev} ./satbias_pc
err2=$?
if [ $err2 -ne 0 ] ; then
  cp $GESROOT_HOLD/satbias_pc ./satbias_pc
fi
cp $nmmb_nems_bias/${RUN}.t${CYCrun}z.radstat.${tmmark_prev}    ./radstat.gdas
err3=$?
if [ $err3 -ne 0 ] ; then
...
cp $nmmb_nems_bias/${RUN}.t${CYCrun}z.abias_air.${tmmark_prev} ./aircftbias_in
err1=$?
if [ $err1 -ne 0 ] ; then
